...
class PerfectedNABARDRecommendationEngine:
    ...
    def __init__(self):
        ...
        self.output_dir = os.path.join("backend", "output") # Add output_dir
    ...
    def analyze_all_farms(self, weather_df: pd.DataFrame, satellite_df: pd.DataFrame,
        ...
        combined_filename = os.path.join(self.output_dir, f"all_perfected_recommendations_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
