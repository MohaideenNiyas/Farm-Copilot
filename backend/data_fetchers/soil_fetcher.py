...
class SoilDataFetcher:
    ...
    def fetch_all_farms_soil(self, farms_file: str, 
        ...
        try:
            ...
            # The raw_soil_dir is now managed by the AgriculturalPipelineController
            # We need to ensure this fetcher has access to it.
            # For now, we'll assume it's handled by the main controller's init
            # and just use the path directly.
            # raw_soil_dir = os.path.join("backend", "raw_soil") # This line is redundant if controller handles it
            # os.makedirs(raw_soil_dir, exist_ok=True) # This line is redundant if controller handles it
            # Instead, we should pass the raw_soil_dir from the controller
            # For now, we'll use the hardcoded path as a fallback if not passed.
            raw_soil_dir = os.path.join("backend", "raw_soil")
            os.makedirs(raw_soil_dir, exist_ok=True)
            ...
            for idx, farm in farms_df.iterrows():
                ...
                cache_file = os.path.join(raw_soil_dir, f"{farm_id}.json")
